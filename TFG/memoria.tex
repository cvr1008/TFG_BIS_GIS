...
alojados en la Unidad de Cuidados Intensivos (UCI) requiere de la
...
retrospectivo en el contexto de la unidad de cuidados intensivos.
...
\item
  ...
  mediante métricas de concordancia (correlación de Pearson, error
  absoluto medio y raíz del error cuadrático medio).
\item
...
o con riesgo vital, que requieren vigilancia estrecha y soporte
continuo.
...
\citep{Kane2017}: - Ciclo: oscilación completa de una onda, desde un
punto inicial hasta que vuelve a alcanzar el mismo estado tras pasar por
un máximo (cresta) y un mínimo (valle). - Amplitud: magnitud de la
variación de la señal respecto a un valor de referencia. En el EEG se
expresa habitualmente en microvoltios (µV). - Frecuencia: número de
ciclos que se producen por segundo. Se expresa en hercios (Hz). -
Potencia de la señal: magnitud empleada en procesamiento de señales para
cuantificar el valor cuadrático medio de sus oscilaciones. Como la
amplitud del EEG se expresa en microvoltios, la potencia de señal se
expresa en microvoltios al cuadrado (\(\mu\mathrm{V}^2\))
\citep{OppenheimSchafer2014}.\footnote{Esta magnitud describe
  ...
  vatios.} - Densidad espectral de potencia: indica cómo se distribuye
la potencia de la señal entre las distintas frecuencias, expresada en
µV²/Hz. Al integrar esta densidad dentro de un intervalo de frecuencias
se obtiene la potencia contenida en esa banda, expresada en µV². -
...
(DSA), la potencia calculada en cada intervalo frecuencial se transforma
a decibelios mediante una comparación logarítmica con una potencia de
referencia. Dicha referencia se obtiene elevando al cuadrado la amplitud
de 0,0001 µV RMS indicada por el fabricante (resultando en \(10^{-8}\)
µV²).
...
frecuencia durante un intervalo determinado. Permite reconocer qué
componentes frecuenciales predominan, pero no muestra cómo cambian a lo
largo del registro.
...
\subsection{Electroencefalografía cuantitativa
(qEEG)}\label{electroencefalografuxeda-cuantitativa-qeeg}

La qEEG hace referencia al conjunto de técnicas que permiten analizar la
señal EEG mediante métodos numéricos y computacionales
\citep{Hwang2022}. A diferencia de la interpretación visual convencional
del trazado, el qEEG permite extraer parámetros objetivos relacionados
con la amplitud, la frecuencia, la potencia o la distribución espectral
de la señal.
...
computacional \citep{OppenheimSchafer2014}. En el análisis de EEG
durante anestesia, este tipo de procesamiento permite calcular los
componentes frecuenciales de señales EEG digitalizadas y construir
representaciones espectrales como el espectrograma \citep{Sawa2022}.
...
general se recogen en la Tabla Tabla~\ref{tbl-bandas-eeg}.
...
El análisis asistido por software permite cuantificar la potencia de las
distintas bandas y representarla mediante herramientas como la matriz de
densidad espectral \citep{Tatum2016}. Por ejemplo, ratio alfa/delta
(ADR) es un índice derivado del EEG que compara la potencia de la banda
alfa con la de la banda delta; un valor más bajo indica un predominio
relativo de actividad lenta, asociado habitualmente a mayor profundidad
de sedación o anestesia \citep{LpezCastruita2023, Hwang2022}.
...
El monitor BIS™ es un sistema avanzado de neuromonitorización diseñado
para evaluar de forma continua el estado hipnótico del cerebro mediante
la adquisición y el procesamiento de señales electroencefalográficas
frontales \citep{CovidienBISadvanced, OchoaSolana2025}. Mediante
algoritmos que analizan la frecuencia, la potencia y la sincronización
de fase del EEG, el sistema genera un índice numérico adimensional
comprendido entre 0 y 100, el BIS \citep{OchoaSolana2025}. Un valor de
100 indica un estado de vigilia completa, mientras que un valor de 0
representa ausencia total de actividad eléctrica cerebral
\citep{CovidienBISadvanced}.

En el ámbito de la anestesia y los cuidados críticos, el rango objetivo
de sedación profunda se sitúa habitualmente entre 40 y 60
\citep{EscuderoAugusto2005}. Mantener el índice dentro de este margen
resulta fundamental para evitar tanto la infrasedación (que incrementa
el riesgo de agitación, desadaptación a la ventilación mecánica y
despertar intraoperatorio) como la sobresedación (índices
consistentemente inferiores a 40), la cual se asocia con un mayor riesgo
de depresión respiratoria, hipoxia e inestabilidad hemodinámica
\citep{PuenteBarbas2016}.
...
\subsection{SEF y MF}\label{sef-y-mf}
...
frecuencial \citep{OchoaSolana2025}: - \textbf{Frecuencia del Borde
Espectral (SEF, Spectral Edge Frequency):} Representada habitualmente
como la SEF95, indica la frecuencia por debajo de la cual se encuentra
el 95 \% de la potencia total del EEG \citep{OchoaSolana2025}. En la DSA
se dibuja como una línea blanca.
...
En el contexto de la UCI, esta información resulta especialmente útil
para reconstruir el contexto clínico en el que se producen determinados
cambios fisiológicos o terapéuticos. Además, distintos estudios han
utilizado datos extraídos de IntelliSpace Critical Care and Anesthesia
para realizar análisis retrospectivos de variables clínicas continuas,
como frecuencia cardiaca o presión arterial, en pacientes críticos
\citep{Laichinger2024, Mengel2025}.
...
La qEEG se ha utilizado en cuidados críticos para transformar registros
...
Array) o la aEEG para facilitar la revisión visual de registros
...
orientados a analizar el EEG mediante espectros y espectrogramas
generados computacionalmente. Por ejemplo, Sawa et al.~presentan una
aproximación basada en Python para calcular y visualizar el espectro de
potencia y el espectrograma del EEG durante anestesia general, mostrando
cómo el procesamiento digital permite representar la evolución
frecuencial de la señal de forma interpretable \citep{Sawa2022}. Este
tipo de trabajos resulta cercano al presente TFG, ya que la
reconstrucción de la DSA requiere transformar señales EEG en
...
\texttt{.r2a} o \texttt{.r4a}, \texttt{.h\_a} y \texttt{.t\_a}. Todos
ellos explicados con detalle en el Anexo F. - El archivo \texttt{.spa}
contiene las variables procesadas por el monitor, como el índice BIS,
SEF, MEF, SQI y otros parámetros de configuración. - El archivo
\texttt{.f\_a} contiene la matriz espectral calculada por el propio
monitor. Este archivo no siempre está disponible, ya que el equipo BIS
VISTA lo exporta vacío. - La cabecera \texttt{.h\_a} proporciona la
pendiente y el offset, información de calibración necesaria para
convertir los valores crudos a microvoltios. - \texttt{.t\_a} aporta el
instante en el que comienza el BIS a registrar. - Los archivos
\texttt{.r2a} y \texttt{.r4a} almacenan el EEG crudo en registros
unilaterales y bilaterales, respectivamente. La diferencia entre ambos
formatos no se limita al nombre del archivo, sino al número de canales
de EEG registrados. Los archivos \texttt{.r2a} contienen dos canales de
señal cruda, mientras que los archivos \texttt{.r4a} contienen cuatro
canales.
...
Las variables clínicas procedentes de ICCA se reciben, directamente del
HUBU, en forma de plantillas Excel estructuradas. Estas plantillas
agrupan la información en hojas diferenciadas: información general del
paciente y del ingreso, constantes vitales, resultados analíticos y
perfusiones o tratamientos administrados. El detalle completo de los
archivos BIS, la estructura de la plantilla ICCA y el significado de las
variables principales se recoge en el Anexo F.

\begin{figure}

\centering{

\includegraphics[width=11.68in,height=4.15in]{memoria_files/figure-latex/mermaid-figure-1.png}

}

\caption{\label{fig-esquema-icca}Organización general de las variables
clínicas procedentes de ICCA.}
...
La Figura Figura~\ref{fig-flujo-dsa} resume el flujo seguido para
obtener o construir la DSA empleada en la aplicación.

\begin{figure}

\centering{

\includegraphics[width=42.04in,height=2.31in]{memoria_files/figure-latex/mermaid-figure-3.png}
...
obtener la matriz en decibelios (\texttt{dB}). Posteriormente, la matriz
se ajusta a la línea temporal de la sesión. Los segundos presentes en la
timeline pero ausentes en la matriz se mantienen como valores no
disponibles, de forma que no se genera continuidad artificial.
...
\includegraphics[width=21.48in,height=2.19in]{memoria_files/figure-latex/mermaid-figure-2.png}

}

\caption{\label{fig-flujo-fa}Flujo de uso de la matriz espectral
original del archivo \texttt{.f\_a}.}
...
el caso de variables sensibles al origen de la muestra, como la pO₂ o la
pCO₂, se aplica un criterio conservador: si no puede determinarse con
seguridad si la medición es arterial o venosa, el dato se descarta para
evitar errores de interpretación. Las perfusiones tampoco se interpolan
como valores acumulados, sino que se representan como cambios
documentados de dosis o velocidad, manteniendo su comportamiento
escalonado.
...
incluyendo la comparación entre métodos espectrales, suavizados,
desplazamientos temporales y métricas cuantitativas, se recogen en el
Anexo G. En este capítulo se resumen los resultados principales
necesarios para valorar el cumplimiento de los objetivos del proyecto.
...
herramienta actúa como punto de entrada al flujo de trabajo
desarrollado, preparando los datos antes de su visualización conjunta.

\begin{figure}

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_portal/captura_menu_portal.png}
...
varias carpetas correspondientes a sesiones BIS. A partir de esta
información, analiza los intervalos temporales de cada fuente y genera
una estructura de almacenamiento organizada por paciente.

\begin{figure}

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_organizador/cap_inicio_organizador.png}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_organizador/cap_crear_paciente.png}

}

\caption{\label{fig-organizador-crear-pac}Creación de un nuevo paciente
a partir de archivos BIS e ICCA.}

\end{figure}%

Antes de confirmar la creación del paciente, la aplicación muestra una
revisión de las asignaciones detectadas. Esto permite comprobar qué
sesiones BIS se asociarán al paciente y qué archivos ICCA se utilizarán
como fuente clínica. Además, la herramienta permite editar asignaciones
ya creadas, facilitando la corrección o actualización de la estructura
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_organizador/cap_revision_paciente.png}

}

\caption{\label{fig-organizador-revisar-pac}Revisión de las asignaciones
antes de crear el paciente.}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_organizador/cap_editar_paciente.png}

}

\caption{\label{fig-organizador-editar-pac}Edición de asignaciones de un
paciente ya creado.}

\end{figure}%

El resultado de este proceso es una organización jerárquica en la que
cada paciente contiene una o varias sesiones BIS. Cada sesión conserva
la exportación BIS original y, cuando existe información clínica
compatible, un archivo ICCA auxiliar recortado al intervalo
correspondiente. Esta separación entre datos originales y derivados
facilita la trazabilidad y permite repetir el procesamiento si se
actualizan los criterios de integración o visualización.

\begin{figure}

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_organizador/cap_analisis_contenido_sesion.png}

}

\caption{\label{fig-organizador-contenido-pac}Contenido generado para
una sesión organizada por paciente.}
...
previamente organizadas. El usuario puede seleccionar un paciente,
escoger una sesión BIS concreta y cargar la información neurofisiológica
y clínica correspondiente a ese intervalo.

\begin{figure}

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_inicio.png}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_paciente_seleccionado.png}

}

\caption{\label{fig-visualizador-seleccionar-pac}Selección de paciente y
sesión BIS en el visualizador.}
...
está disponible y contiene información válida, la aplicación utiliza
directamente la matriz espectral exportada por el monitor. En cambio,
cuando este archivo no existe o está vacío, la DSA se reconstruye a
partir de la señal EEG cruda almacenada en \texttt{.r2a} o
\texttt{.r4a}.
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_DSA_reconstruida_unilateral.png}

}

\caption{\label{fig-visualizador-dsa-rec-unilat}Visualización de una DSA
unilateral reconstruida a partir del EEG crudo.}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_DSA_fa_unilateral.png}

}

\caption{\label{fig-visualizador-dsa-fa-unilat}Visualización de una DSA
unilateral obtenida desde el archivo espectral \texttt{.f\_a}.}

\end{figure}%

Las Figuras Figura~\ref{fig-visualizador-dsa-rec-unilat} y
Figura~\ref{fig-visualizador-dsa-fa-unilat} muestran ambos escenarios en
un registro unilateral: la reconstrucción desde el EEG crudo y la
visualización de la matriz espectral exportada por el monitor. Esta
doble posibilidad resulta importante porque no todas las sesiones
disponibles contienen un archivo \texttt{.f\_a} válido.

También se comprobó el funcionamiento del sistema en registros
procedentes de BIS VISTA en los que no se dispone de información clínica
ICCA asociada y no existe una matriz \texttt{.f\_a} válida. En estos
casos, el visualizador permite cargar igualmente la sesión BIS y fuerza
la reconstrucción de la DSA a partir de la señal cruda.

\begin{figure}

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_sinICCA_sinFA.png}

}

\caption{\label{fig-visualizador-seleccionar-pac-vista}Selección de una
sesión BIS VISTA sin ICCA asociado y sin archivo \texttt{.f\_a} válido.}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_VISTA_bilat.png}

}

\caption{\label{fig-visualizador-dsa-rec-bilat}Reconstrucción bilateral
de la DSA en un registro BIS VISTA sin archivo \texttt{.f\_a} válido.}
...
correspondientes a los hemisferios izquierdo y derecho. La Figura
Figura~\ref{fig-visualizador-dsa-rec-bilat} muestra el resultado de esta
...
La aplicación aplica criterios de calidad para evitar la representación
...
perfusiones. Esta separación permite representar cada tipo de variable
de acuerdo con su naturaleza temporal.

Las constantes vitales se trataron como series temporales asociadas al
intervalo de la sesión BIS. Los análisis clínicos se representaron como
mediciones puntuales, conservando su instante de registro. Las
perfusiones se incorporaron como cambios de dosis activa a lo largo del
tiempo, lo que permite estudiar su relación temporal con la evolución
del BIS y de la DSA.
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_variables_clinicas.png}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_tarjetas_analisis.png}

}

\caption{\label{fig-visualizador-analisis-icca}Visualización de
resultados analíticos asociados a la sesión BIS.}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_perfusiones_2.png}

}

\caption{\label{fig-visualizador-perfusiones-icca}Representación de
perfusiones y tratamientos durante el intervalo analizado.}
...
El resultado final del proyecto es un visualizador interactivo que
permite explorar registros BIS unilaterales y bilaterales junto con la
información clínica disponible. La interfaz permite seleccionar paciente
y sesión, modificar la ventana temporal visible y revisar periodos
concretos del registro.

En registros bilaterales, el visualizador muestra de forma diferenciada
la información correspondiente a ambos hemisferios, incluyendo las
matrices DSA izquierda y derecha, las curvas BIS asociadas y la
asimetría bilateral. Además, permite incorporar variables clínicas
seleccionadas para facilitar el análisis retrospectivo de situaciones
concretas del paciente crítico.
...
los datos identificativos de la sesión (paciente, sesión BIS, tipo de
registro, origen de la DSA, intervalo temporal analizado y
disponibilidad de información ICCA), una página con la representación
completa de la DSA junto con las variables procesadas del monitor, y un
resumen textual de la información clínica ICCA asociada al intervalo
(constantes vitales, análisis clínicos y perfusiones), cuando está
disponible.
...
Las Figuras Figura~\ref{fig-informe-pdf},
Figura~\ref{fig-informe-dsa-pdf} y Figura~\ref{fig-informe-analisis-pdf}
muestran, respectivamente, la portada del informe, la página con la
representación de la DSA y el resumen de la información clínica ICCA.

\begin{figure}

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_informe_pdf.png}

}

\caption{\label{fig-informe-pdf}Portada del informe PDF, con los datos
identificativos de la sesión.}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_informe_2_pdf.png}

}

\caption{\label{fig-informe-dsa-pdf}Página de la DSA y las variables
procesadas del monitor en el informe PDF.}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/capturas_app_visualizador/visual_informe_5_pdf.png}

}

\caption{\label{fig-informe-analisis-pdf}Resumen de análisis clínicos
ICCA en el informe PDF.}
...
En cuanto a los datos procedentes de ICCA, la aplicación parte de
plantillas estructuradas proporcionadas por personal clínico autorizado.
Por tanto, el trabajo no aborda la extracción directa desde la base de
datos de ICCA, sino la integración posterior de los datos recibidos.
Esto limita el alcance del sistema, ya que la disponibilidad de
variables depende de la plantilla de exportación y de los criterios de
