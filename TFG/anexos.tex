...
% Necesario para \includepdf en el Anexo J (documentación CEIm)
\usepackage{pdfpages}
...
Este proyecto surge de la necesidad, detectada en la Unidad de Cuidados
Intensivos del Hospital Universitario de Burgos (HUBU), de facilitar el
análisis retrospectivo de pacientes críticos combinando dos fuentes de
información que hasta ahora se consultaban por separado: la
neuromonitorización del monitor BIS y las variables clínicas registradas
en el sistema ICCA. Ambas fuentes tienen resoluciones temporales muy
distintas y formatos propios, lo que dificulta relacionar la evolución
del BIS y de la DSA con el contexto clínico del paciente en un mismo
momento.

El alcance del trabajo se limita deliberadamente a una prueba de
concepto: no se pretende sustituir la interpretación clínica ni ofrecer
un producto validado para uso asistencial, sino demostrar que es viable
organizar, sincronizar y visualizar conjuntamente ambos tipos de datos.
Para ello se han desarrollado tres piezas complementarias (un portal de
acceso, una aplicación de organización de pacientes y sesiones, y un
visualizador interactivo), apoyadas en un flujo de reconstrucción de la
matriz de densidad espectral cuando el archivo espectral original no
está disponible.

\chapter{Metodología de ingeniería del
software}\label{metodologuxeda-de-ingenieruxeda-del-software}

El desarrollo no siguió una metodología formal predefinida (no se
emplearon, por ejemplo, sprints de Scrum ni un ciclo en cascada
completo), sino un enfoque incremental basado en prototipos, adoptado
principalmente porque la disponibilidad real de los datos condicionó el
orden de trabajo: los datos clínicos reales del HUBU no se obtuvieron
hasta las últimas semanas del proyecto, por lo que fue necesario diseñar
y validar buena parte del sistema con datos ficticios antes de disponer
de los reales.
...
\tightlist
\item
  \textbf{Exploración inicial.} Se estudió la estructura de los archivos
  BIS (\texttt{.spa}, \texttt{.f\_a}, \texttt{.r2a}/\texttt{.r4a},
  \texttt{.h\_a}, \texttt{.t\_a}) mediante \emph{notebooks}
  exploratorios, implementando funciones de lectura y decodificación
  como primer contacto con los datos.
\item
  \textbf{Consolidación en módulos.} Las funciones exploratorias se
  reorganizaron en módulos reutilizables (\texttt{reconstruccion.py},
  \texttt{lectura\_spa.py}, \texttt{lectura\_fa.py},
  \texttt{alineacion\_temporal.py}, entre otros), separando la lógica de
  procesamiento de la interfaz.
\item
  \textbf{Desarrollo de las aplicaciones.} Sobre esos módulos se
  construyeron las tres aplicaciones Dash (portal, organizador y
  visualizador), cada una con su propio ciclo de prueba.
\item
  \textbf{Ciclos de prueba y ajuste.} Cada iteración se comprobó con
  registros reales unilaterales y bilaterales, revisando visualmente las
  salidas y, cuando existía un archivo \texttt{.f\_a} disponible,
  validando cuantitativamente la reconstrucción frente a él (Anexo G).
\item
  \textbf{Adaptación a los datos reales.} La llegada progresiva de los
  archivos clínicos reales obligó a ajustar de nuevo las visualizaciones
  y algunos criterios de lectura, ya definidos inicialmente sobre datos
  ficticios.
\end{enumerate}

En cuanto a la trazabilidad y reproducibilidad, se mantuvieron siempre
separados los archivos originales de los derivados generados por las
aplicaciones, y los procesos de reconstrucción y organización son
deterministas: repetirlos sobre los mismos archivos de entrada produce
los mismos resultados.

Dado que este apéndice recoge de forma centralizada las fases seguidas,
no se ha considerado necesario anotar de nuevo la fase correspondiente
en cada uno de los apéndices B a G.

\chapter{Planificación temporal}\label{planificaciuxf3n-temporal}

La planificación temporal del proyecto estuvo condicionada por la
disponibilidad progresiva de los datos clínicos reales. Por este motivo,
el desarrollo se organizó de forma incremental: primero se trabajó con
datos ficticios y registros BIS disponibles para construir la
arquitectura general del sistema, y posteriormente se adaptó el flujo a
los datos reales procedentes del HUBU una vez obtenida la aprobación
ética.
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{img/planificacion_temporal.png}

}

\caption{\label{fig-planificacion-temporal}Planificación temporal real
del proyecto.}

\end{figure}%

En la Figura Figura~\ref{fig-planificacion-temporal} se recoge el
cronograma real del proyecto, diferenciando las fases de desarrollo
técnico, documentación, tramitación ética, entrega y preparación de la
defensa.

\chapter{Planificación económica}\label{planificaciuxf3n-econuxf3mica}

La planificación económica se plantea como una estimación del coste
equivalente del proyecto, no como un coste facturado real. La mayor
parte de herramientas utilizadas son de libre acceso o cuentan con
licencia institucional, por lo que el coste principal corresponde al
...
\caption{Estimación económica del
proyecto}\label{tbl-planificacion-economica}\tabularnewline
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
Personal & 300 horas de desarrollo, análisis, pruebas y documentación,
estimadas a 15 €/h & 4.500 € \\
Hardware & Amortización parcial del equipo clínico empleado durante el
desarrollo & 125 € \\
Software & Python, Dash, Plotly, Quarto, R, GitHub y librerías de
análisis empleadas sin coste directo & 0 € \\
Documentación y almacenamiento & Material de documentación,
almacenamiento local y copias auxiliares & 30 € \\
Otros costes indirectos & Electricidad, conexión a red y uso general del
equipo & 50 € \\
\textbf{Total estimado} & & \textbf{4705 €} \\
\end{longtable}

\chapter{Viabilidad técnica}\label{viabilidad-tuxe9cnica}

La viabilidad técnica del proyecto se apoya en el uso de herramientas
abiertas y ampliamente utilizadas en análisis de datos biomédicos y
desarrollo de aplicaciones interactivas. El procesamiento de datos se
realizó principalmente en Python, utilizando librerías para lectura,
transformación, análisis numérico y visualización. La interfaz se
desarrolló mediante Dash y Plotly, lo que permitió construir una
aplicación web local sin necesidad de desplegar inicialmente el sistema
en un servidor hospitalario.

El proyecto también se diseñó separando la lógica de procesamiento de
datos de la interfaz gráfica. Esta decisión facilita la reutilización de
los módulos de lectura, alineación temporal, reconstrucción de DSA y
visualización, y permite modificar partes concretas del sistema sin
rehacer toda la aplicación.

Para reducir problemas de compatibilidad entre equipos, versiones de
Python y dependencias, el sistema puede ejecutarse en un entorno
controlado, como un entorno virtual o una máquina virtual configurada
con las librerías necesarias. Esta posibilidad resulta especialmente
útil si en el futuro se desea reproducir el proyecto en otro equipo o
trasladarlo a un entorno más cercano al hospitalario.

\chapter{Gestión de riesgos}\label{gestiuxf3n-de-riesgos}

Durante el desarrollo se identificaron varios riesgos técnicos y
organizativos. La Tabla Tabla~\ref{tbl-riesgos} resume los principales
riesgos detectados y las medidas adoptadas para reducir su impacto.

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3333}}@{}}
\caption{Riesgos principales del proyecto y medidas
adoptadas}\label{tbl-riesgos}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Riesgo
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\bottomrule\noalign{}
\endlastfoot
Retraso en la disponibilidad de datos reales & Dificultad para probar el
flujo completo desde el inicio & Desarrollo inicial con datos ficticios
y adaptación posterior \\
Ausencia o invalidez del archivo \texttt{.f\_a} & Imposibilidad de
visualizar la DSA original & Implementación de reconstrucción desde EEG
crudo \\
Algoritmo BIS propietario & Imposibilidad de reproducir exactamente el
procesamiento interno del monitor & Validación comparativa frente a
\texttt{.f\_a} cuando está disponible \\
Heterogeneidad temporal entre BIS e ICCA & Dificultad para representar
todas las variables en una línea temporal común & Diseño de una timeline
basada en la sesión BIS y representación diferenciada por tipo de
variable \\
Restricciones éticas y de protección de datos & Limitación en el acceso
y tratamiento de datos clínicos & Tramitación de aprobación ética y
trabajo con datos bajo condiciones autorizadas \\
Calidad variable de la señal BIS & Posible representación de tramos no
interpretables & Incorporación de máscaras de calidad y detección de
discontinuidades \\
\end{longtable}

\chapter{Viabilidad legal}\label{viabilidad-legal}

El proyecto se ha desarrollado con datos clínicos procedentes del
Hospital Universitario de Burgos (HUBU). Su uso fue autorizado por el
Comité de Bioética correspondiente, cuya aprobación fue concedida el 2
de julio de 2026, habilitando el tratamiento de información clínica
retrospectiva bajo condiciones de confidencialidad, protección de datos
y finalidad investigadora.

El trabajo se limita a la organización, integración temporal y
visualización retrospectiva de los datos recibidos. La extracción de las
variables clínicas procedentes de ICCA fue realizada por personal
autorizado, y la aplicación desarrollada no accede directamente a la
base de datos hospitalaria ni modifica los datos originales.

Esta aprobación ampara el desarrollo y la prueba de concepto descritos
en este trabajo, pero no constituye una validación clínica de la
herramienta ni autoriza su uso como apoyo a la toma de decisiones
asistenciales. El sistema debe entenderse como una herramienta
experimental orientada al análisis retrospectivo y a la exploración de
datos.
...
\chapter{Diseño arquitectónico}\label{diseuxf1o-arquitectuxf3nico}
...
\section{Arquitectura lógica de la
...
\section{Diseño de despliegue}\label{diseuxf1o-de-despliegue}
...
\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{img/esquema_arquitectura_hubu.png}
...
\section{Organización de los datos}\label{organizaciuxf3n-de-los-datos}
...
\NormalTok{|   \textasciigrave{}{-}{-} SESIONES/}
\NormalTok{|       |{-}{-} SESION\_001\_...}
\NormalTok{|       |   |{-}{-} sesion.json}
\NormalTok{|       |   |{-}{-} BIS/}
\NormalTok{|       |   \textasciigrave{}{-}{-} ICCA/}
\NormalTok{|       \textasciigrave{}{-}{-} SESION\_002\_...}
\NormalTok{|           |{-}{-} sesion.json}
\NormalTok{|           |{-}{-} BIS/}
\NormalTok{|           \textasciigrave{}{-}{-} ICCA/}
\NormalTok{\textasciigrave{}{-}{-} PACIENTE\_002/}
...
\section{Organización interna del
...
\section{Principales módulos del
...
\section{Justificación del diseño}\label{justificaciuxf3n-del-diseuxf1o}
...
\chapter{Diagrama de casos de uso}\label{diagrama-de-casos-de-uso}
...
\chapter{Explicación de los casos de
...
\chapter{Prototipos de interfaz o interacción con el
...
\section{CU-1: Crear paciente}\label{cu-1-crear-paciente}
...
\section{CU-3: Visualizar sesión
...
\section{CU-4: Generar informe PDF}\label{cu-4-generar-informe-pdf}
...
\chapter{Requisitos software y hardware para ejecutar el
...
\chapter{Instalación y puesta en
...
\section{Ejecución local}\label{ejecuciuxf3n-local}
...
\section{Ejecución en entorno con máquinas
...
\chapter{Manuales y demostraciones
prácticas}\label{manuales-y-demostraciones-pruxe1cticas}

\section{Acceso al portal principal}\label{acceso-al-portal-principal}
...
\section{Visualizar paciente}\label{visualizar-paciente}
...
\section{Exportación de informe
...
\chapter{Consideraciones de uso}\label{consideraciones-de-uso}
...
\chapter{Estructura de directorios}\label{estructura-de-directorios}
...
\NormalTok{|   |   \textasciigrave{}{-}{-} requirements.txt}
...
\NormalTok{|   \textasciigrave{}{-}{-} detener\_aplicaciones.ps1}
\NormalTok{|{-}{-} datos/}
\NormalTok{|   \textasciigrave{}{-}{-} pacientes/}
\NormalTok{|{-}{-} README.md}
\NormalTok{\textasciigrave{}{-}{-} .gitignore}
\end{Highlighting}
\end{Shaded}
...
\chapter{Compilación, instalación y ejecución del
...
\chapter{Pruebas del sistema}\label{pruebas-del-sistema}
...
\chapter{Instrucciones para la modificación o mejora del
...
\chapter{Aprobación ética y marco de uso de los
...
(CEIm) correspondiente, cuya aprobación (dictamen CEIm nº 3592) fue
concedida el 2 de julio de 2026. En el Anexo J se incluyen tanto la
solicitud como la resolución favorable.
...
\chapter{Origen y adquisición de los datos
...
los electrodos 3 y 4 como puntos de lectura \citep{CovidienBISadvanced}.
De esta disposición se obtienen el canal 1, como diferencia entre los
electrodos 1 y 3, y el canal 2, como diferencia entre los electrodos 1 y
4.
...
(\texttt{RT}) \citep{CovidienBISadvanced}. Los canales 1 y 2 se obtienen
en el lado izquierdo y los canales 3 y 4 en el derecho.

De forma análoga al sensor unilateral, cada canal bilateral se obtiene
como la diferencia de potencial entre el electrodo central \texttt{C}
(positivo, compartido por los cuatro canales) y cada uno de los cuatro
electrodos de lectura (negativos): el canal 1 corresponde a la
diferencia entre \texttt{C} y \texttt{LT}, el canal 2 a la diferencia
entre \texttt{C} y \texttt{LE}, el canal 3 a la diferencia entre
\texttt{C} y \texttt{RT}, y el canal 4 a la diferencia entre \texttt{C}
y \texttt{RE} \citep{CovidienExportData}. Los canales elegidos para la
reconstrucción en este proyecto, el 1 y el 3, corresponden por tanto a
los dos electrodos temporales (\texttt{LT} y \texttt{RT}), uno por
hemisferio.
...
\chapter{Organización de una exportación
...
EEG. Las extensiones y el contenido de cada archivo de exportación
siguen la convención documentada por el fabricante
\citep{CovidienBISadvanced}.
...
No todos los equipos producen archivos \texttt{.f\_a} con contenido. En
concreto, el equipo BIS VISTA exporta sistemáticamente un archivo
\texttt{.f\_a} de cero bytes: no se trata de una incidencia ocasional de
algunos registros, sino de un comportamiento propio de este modelo de
monitor, a diferencia del BIS Advance, cuyas exportaciones sí contienen
la matriz espectral. Esta circunstancia es relevante para el proyecto,
porque justifica por sí sola la necesidad de reconstruir la DSA a partir
del EEG crudo para todos los registros procedentes de BIS VISTA.

\chapter{\texorpdfstring{Archivos de EEG crudo: \texttt{.r2a} y
...
\chapter{Decodificación y calibración de las
...
\section{Diferencias entre unilateral y
...
\chapter{\texorpdfstring{Archivo de cabecera:
...
\chapter{\texorpdfstring{Archivo temporal:
...
\chapter{\texorpdfstring{Archivo de espectros:
...
\chapter{\texorpdfstring{Archivo de variables procesadas:
...
\section{Estructura unilateral}\label{estructura-unilateral}
...
\section{Estructura bilateral}\label{estructura-bilateral}
...
\chapter{Variables procesadas de mayor
...
\section{\texorpdfstring{Índice BIS:
...
\section{\texorpdfstring{Calidad de señal:
...
\section{\texorpdfstring{Potencia total:
...
\section{Frecuencias SEF y MEF}\label{frecuencias-sef-y-mef}
...
\section{\texorpdfstring{Suavizado espectral:
...
\section{\texorpdfstring{Filtro de baja frecuencia:
...
\section{\texorpdfstring{Asimetría bilateral:
...
\chapter{\texorpdfstring{Otras variables disponibles en
...
\chapter{Preparación y validación de los
...
\chapter{Reconstrucción desde el EEG
...
\chapter{Plantilla ICCA: estructura y
...
SpO\(_2\), temperatura, frecuencia respiratoria, PIC & Mediciones
periódicas \\
...
\chapter{Organización de pacientes y
...
\section{Detección de sesiones BIS y de plantillas ICCA
...
\section{Cálculo de solapamiento y compatibilidad
...
\section{Estructura de carpetas y
...
\section{Generación del Excel ICCA
...
\section{Validaciones y robustez}\label{validaciones-y-robustez}
...
\chapter{Preparación de datos ICCA para el
...
\section{Generación de series sintéticas a resolución de un
...
\section{Auditoría e inferencia del tipo de
...
si la muestra es arterial o venosa para interpretarse correctamente, y
esta información no siempre consta de forma explícita en ICCA. La
aplicación resuelve esta situación mediante un procedimiento de
auditoría que, para cada determinación, decide si puede incluirse en la
visualización y con qué grado de confianza:
...
\item
  ...
  sin su pareja pO\(_2\)/pCO\(_2\) en el mismo instante, se excluye por
  considerarse incompleta.
\item
  Si pO\(_2\) y pCO\(_2\) aparecen juntas en el mismo instante y las
  marcas asociadas a la medición son coherentes entre sí, el tipo de
  muestra puede inferirse con confianza alta a partir de ese contexto.
\item
...
\chapter{Consideraciones clínicas y
...
\chapter{G.1 Objetivo del anexo}\label{g.1-objetivo-del-anexo}
...
\chapter{G.2 Parámetros generales de
...
\texttt{umbral\_ceros\_raw} & 0,5 & Segundo de EEG crudo inválido si
\$\geq\$50 \% de sus muestras son cero \\
...
\chapter{G.3 Lectura de cabecera, tiempo inicial y onda
...
\chapter{G.4 Conversión de la señal cruda a
...
\citep{OppenheimSchafer2014}, implementado con
\texttt{scipy.signal.butter} (\texttt{output="sos"}) y aplicado mediante
\texttt{sosfilt}. Al ser causal, cada muestra de salida depende
únicamente de muestras presentes y anteriores, respetando la dirección
...
mediante transformación bilineal \citep{OppenheimSchafer2014}, los
coeficientes pueden expresarse a partir de:
...
\chapter{G.6 Reconstrucción espectral mediante
...
La estimación espectral se realiza mediante el método de Welch
\citep{Welch1967}, que promedia periodogramas modificados calculados
sobre segmentos solapados de la señal para reducir la varianza de la
estimación respecto a un periodograma único sobre toda la señal.
...
\section{De la señal discreta a la resolución de 0,5
...
muestreadas y finitas, la Transformada Discreta de Fourier (DFT)
\citep{OppenheimSchafer2014}; la FFT (\emph{Fast Fourier Transform}) no
es una transformada distinta, sino el algoritmo con el que se calcula
esa misma DFT de forma mucho más rápida, y es lo que usa internamente
...
\section{Integración por bin y paso a
...
La referencia del fabricante se expresa como una amplitud, 0,0001 µV RMS
(\emph{Root Mean Square}, valor eficaz o cuadrático medio de la señal),
mientras que lo que se compara en la fórmula es una potencia. Por ello
la referencia debe elevarse al cuadrado,
...
94 dB especificada por el fabricante
\citep{CovidienBISVista, CovidienVISTABilateral}. Los valores que quedan
...
\chapter{G.8 Alineación temporal de la DSA
...
\chapter{G.9 Máscaras de calidad}\label{g.9-muxe1scaras-de-calidad}
...
Pérdida de señal cruda & \$\geq\$50 \% de las muestras del segundo, en
el canal correspondiente, son exactamente cero \\
...
\chapter{\texorpdfstring{G.10 Suavizado temporal mediante
...
\chapter{G.11 Desplazamiento temporal
...
\chapter{G.12 Aplicación final de máscaras y variables
...
\chapter{\texorpdfstring{G.13 Funciones principales del módulo
...
\chapter{G.15 Resultados
experimentales}\label{g.15-resultados-experimentales}

\section{\texorpdfstring{G.15.1 Equivalencia entre Welch y
\texttt{spectrogram}, y comparación con
\emph{wavelet}}{G.15.1 Equivalencia entre Welch y spectrogram, y comparación con wavelet}}\label{g.15.1-equivalencia-entre-welch-y-spectrogram-y-comparaciuxf3n-con-wavelet}
...
idénticos: de las 682 combinaciones de filtro, suavizado, ventana y
desplazamiento evaluadas en la validación experimental, las 682
arrojaron exactamente el mismo Pearson, MAE y RMSE para ambas funciones.
Por tanto, \texttt{spectrogram} no constituyó una metodología espectral
distinta en estas condiciones, sino otra interfaz para organizar los
mismos periodogramas modificados descritos en G.6. Se mantuvo la
denominación Welch por describir con mayor precisión el estimador
utilizado, coherente con otros trabajos que también implementan el
análisis espectral del EEG mediante Python mediante estas dos funciones
equivalentes \citep{Sawa2022}.

En la misma validación se probó adicionalmente una transformada wavelet
como alternativa a Welch. Aunque el Pearson obtenido con wavelet resultó
razonable (hasta 0,69 en su mejor configuración), el error en escala
física fue muy superior al de Welch: MAE en torno a 18,8 dB y RMSE en
torno a 19,3 dB, frente a los 3,7 y 5,0 dB de Welch. Esta diferencia tan
marcada en el error absoluto, pese a una correlación de patrón similar,
descartó la transformada wavelet como alternativa viable para este
proyecto.

\begin{longtable}[]{@{}lrrr@{}}
\caption{Comparación de métodos espectrales, mejor configuración de cada
uno (registro
\protect\texttt{L03041035})}\label{tbl-metodos-espectrales}\tabularnewline
\toprule\noalign{}
...
\texttt{spectrogram} & 0,726 & 3,72 & 5,02 \\
Wavelet & 0,690 & 18,75 & 19,26 \\
\end{longtable}

\section{G.15.2 Comparación de filtrado y tipos de
suavizado}\label{g.15.2-comparaciuxf3n-de-filtrado-y-tipos-de-suavizado}

La misma validación experimental permitió comparar, sobre el mismo
registro y con el resto de parámetros fijos, tres decisiones
metodológicas adoptadas en G.5 y G.10: aplicar o no el filtro pasa-altos
Butterworth, y qué forma de suavizado temporal usar.

\textbf{Filtro pasa-altos (Butterworth).} Sobre la mejor combinación de
ventana y desplazamiento en cada caso (suavizado \texttt{rolling},
ventana 30 s):

\begin{longtable}[]{@{}lrrrr@{}}
\caption{Efecto del filtro pasa-altos sobre la correspondencia con la
DSA original}\label{tbl-butterworth-comparacion}\tabularnewline
\toprule\noalign{}
...
Aplicar el filtro reduce el RMSE en más de 1,4 dB (un 22 \% de mejora
relativa) y el MAE en 0,8 dB, además de mejorar ligeramente el Pearson.
Esto respalda la decisión de aplicar sistemáticamente el filtro
pasa-altos antes del cálculo espectral (G.5), y no solo como un ajuste
opcional.

\textbf{Tipo de suavizado temporal.} Comparando, para cada tipo de
suavizado, su mejor combinación de ventana y desplazamiento (con filtro
Butterworth aplicado):

\begin{longtable}[]{@{}lrrrrr@{}}
\caption{Comparación de estrategias de suavizado
temporal}\label{tbl-suavizado-comparacion}\tabularnewline
\toprule\noalign{}
...
Media móvil causal (\texttt{rolling}) & 30 & 11 & 0,726 & 3,718 &
5,021 \\
Media móvil exponencial (\texttt{ewm}) & 30 & 11 & 0,694 & 3,813 &
5,161 \\
\end{longtable}

No suavizar la señal deja un RMSE muy superior (8,58 dB) al de
cualquiera de las dos formas de suavizado probadas, lo que confirma que
el suavizado temporal es necesario y no una simple opción estética.
Entre las dos formas de suavizado con memoria, la media móvil causal
(\texttt{rolling}), adoptada finalmente en el proyecto (G.10), obtuvo
mejores resultados que la media móvil exponencial en las tres métricas,
aunque la diferencia entre ambas es más moderada que frente a no
suavizar en absoluto.

Cabe destacar que, en las tres comparaciones anteriores (Welch frente a
\emph{wavelet} en G.15.1, Butterworth aplicado o no, y los tres tipos de
suavizado), la combinación finalmente adoptada por el proyecto ---Welch,
filtro Butterworth aplicado, suavizado \texttt{rolling}--- coincide en
los tres casos con la opción de mejor resultado experimental, y no solo
con una elección razonable a priori.

\section{G.15.3 Resultados de la validación por
registro}\label{g.15.3-resultados-de-la-validaciuxf3n-por-registro}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{L03041035} & Unilateral & 10 & 0,7260 & 3,711 & 5,022 & 7,8
\% \\
...
\textbf{Promedio} & & & \textbf{0,9154} & \textbf{1,815} &
\textbf{2,578} & \textbf{2,9 \%} \\
\end{longtable}
...
\section{G.15.4 Configuración
final}\label{g.15.4-configuraciuxf3n-final}
...
simplicidad metodológica; y, como se detalla en G.15.1 y G.15.2,
coincide en cada una de las decisiones comparadas (método espectral,
filtro y suavizado) con la opción experimentalmente superior.
...
\chapter{G.16 Limitaciones}\label{g.16-limitaciones}
...
\chapter{Sostenibilidad social: eficiencia y bienestar del personal
...
\chapter{Responsabilidad ética en el uso de
...
información. La necesidad de tramitar y obtener la aprobación del CEIm
antes de poder usar datos reales, y de mantener en todo momento la
separación entre archivos originales y derivados, la trazabilidad de las
fuentes y la ausencia de identificadores directos del paciente, me ha
hecho entender que la innovación tecnológica en salud solo es sostenible
si se construye sobre una base de confianza y de respeto a los derechos
de las personas cuyos datos se utilizan. Esta experiencia ha reforzado
mi conciencia de que no toda mejora técnica es deseable si no se
acompaña de garantías éticas adecuadas, y que el tiempo dedicado a estos
aspectos (que en ocasiones puede parecer que ralentiza el desarrollo)
es, en realidad, parte imprescindible de un proyecto responsable.

\chapter{Sostenibilidad tecnológica y
...
A esto se añade una dimensión de sostenibilidad que valoro
especialmente: la de evitar que otras personas tengan que repetir desde
cero un trabajo de exploración ya realizado. La estructura de los
archivos que exporta el monitor BIS (\texttt{.spa}, \texttt{.f\_a},
\texttt{.r2a}/\texttt{.r4a}, \texttt{.h\_a}, \texttt{.t\_a}, entre
otros) estaba muy poco documentada de forma accesible antes de este
trabajo; la exploración detallada que ha exigido este proyecto, recogida
en el Anexo F, puede ahorrar ese mismo esfuerzo a quien continúe esta
línea de trabajo en el futuro. De manera similar, la metodología de
reconstrucción de la matriz espectral (Anexo G) no pretende ser una
réplica exacta del algoritmo propietario del monitor, sino una
aproximación validada y documentada que sienta una base sobre la que
apoyarse: quien quiera mejorarla, adaptarla a otro equipo o compararla
con nuevas alternativas parte de un punto de partida ya construido, en
lugar de empezar de cero.

\chapter{Reflexión final}\label{reflexiuxf3n-final}
